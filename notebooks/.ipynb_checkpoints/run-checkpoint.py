...
def run_corona_mundo():
    ...
    df_wordmeters = manipulation.create_all_country_total_data(
        df_wordmeters, vars=["confirmed", "deaths", "recovered", "active"]
    )
    ...
    for var in themes["vars"].keys():
        for scale in themes["axis_legend"]["scale"].keys():
            fig = vis_graphs.all_countrys(
                df[mask_countrys], var, scale, themes, save=False
            )

            #         plot(fig, filename=f"../images/multipleCountry/{var}_{scale}.html", auto_open=False)
            plot(
                fig,
                filename=f"../../sample_pages/pages/covid-19/images/multipleCountry/{var}_{scale}.html",
                auto_open=False,
            )
    ...
    for countrycode in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/singleCountry/{countrycode}_daily.html",
            auto_open=False,
        )
    ...
    for countrycode in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/singleCountry/{countrycode}_cum.html",
            auto_open=False,
        )
...
def run_corona_br():
    ...
    for var in themes["vars"].keys():
        for scale in themes["axis_legend"]["scale"].keys():
            ...
            plot(
                fig,
                filename=f"../../sample_pages/pages/covid-19/images/brasil/cumulative/{var}_{scale}.html",
                auto_open=False,
            )
    ...
    for state in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/brasil/daily/{state}_daily.html",
            auto_open=False,
        )
    ...
    for state in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/brasil/daily/{state}_cum.html",
            auto_open=False,
        )
    ...
    for var in themes["vars"].keys():
        for scale in themes["axis_legend"]["scale"].keys():
            ...
            plot(
                fig,
                filename=f"../../sample_pages/pages/covid-19/images/vale/cumulative/{var}_{scale}.html",
                auto_open=False,
            )
    ...
    for state in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/vale/daily/{unidecode.unidecode(state.replace(' ','_').lower())}_daily.html",
            auto_open=False,
        )
    ...
    for state in codes:
        ...
        plot(
            fig,
            filename=f"../../sample_pages/pages/covid-19/images/vale/daily/{unidecode.unidecode(state.replace(' ','_').lower())}_cum.html",
            auto_open=False,
        )
...
def run_coronaBR():
    ...
    br = manipulation.create_all_country_total_data(
        br, vars=["confirmed", "deaths", "recovered", "active"]
    )
    ...
    themes = yaml.load(
        open("../themes/custom_colorscales.yaml", "r"), Loader=yaml.FullLoader
    )
    themes = themes["brasil_vis_cumulative"]
    ...
    for var in themes["vars"].keys():
        for scale in themes["axis_legend"]["scale"].keys():
            ...
            plot(
                fig,
                filename=f"../site/sv_br_vale/coronavirus/images/cumulative/brasil_por_estado_{var}.html",
                auto_open=False,
            )
    ...
    for state in codes:
        ...
        plot(
            fig,
            filename=f"../site/sv_br_vale/coronavirus/images/daily/{state}.html",
            auto_open=False,
        )
    ...
    final, estados = manipulation.manipulate_for_br_maps(
        brio, municipios, maps, df_states.drop(["regiao"], 1), ufs
    )
    ...
    vale, estados = manipulation.manipulate_for_vale_maps(
        df, municipios_sp, estados, config["vale_map"]
    )
    ...
    credentials = yaml.load(
        open("../../credentials/ftp_credentials.yaml", "r"), Loader=yaml.FullLoader
    )
...
def run_coronaVale():
    ...
    sp_full["nome_drs"] = np.where(
        sp_full["nome_munic"] == "São Paulo",
    ...
    vale_ = vale_.rename(columns=rename_cols).drop(
        columns=["nome_munic", "codigo_ibge"]
    )
    ...
    vale_final["casos_novos_tbt"] = (
        100 * vale_final["casos_novos"] / vale_final["casos_novos_tbt"]
    )
    vale_final["obitos_tbt"] = 100 * vale_final["obitos"] / vale_final["obitos_tbt"]
    vale_final["obitos_novos_tbt"] = (
        100 * vale_final["obitos_novos"] / vale_final["obitos_novos_tbt"]
    )
    ...
    casos_full = pd.read_csv(
        "https://raw.githubusercontent.com/seade-R/dados-covid-sp/master/data/dados_covid_sp.csv",
        sep=";",
    )
    casos, sp_casos = manipulation_sp.padronize_casos(casos_full)

    internacoes_full = pd.read_csv(
        "https://github.com/seade-R/dados-covid-sp/raw/master/data/plano_sp_leitos_internacoes.csv",
        sep=";",
    )
    internacoes, sp_internacoes = manipulation_sp.padronize_internacoes(
        internacoes_full
    )
