...
def load_brasilIO():
    ...
    mask = (
        (brio["is_last"] == True)
        & (brio["place_type"] == "city")
        & (brio["last_available_confirmed_per_100k_inhabitants"].notnull())
    )
    cols = [
        "city_ibge_code",
        "city",
...
def update_ms_data():
    ...
    profile.set_preference(
        ...
        "application/x-rar-compressed,application/csv,application/excel,application/vnd.msexcel,application/vnd.ms-excel,text/anytext,text/comma-separated-values,text/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/octet-stream",
    )
